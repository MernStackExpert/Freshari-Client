...
const ProductDetailsPage = () => {
  ...
  if (loading)
    return (
      <div className="h-screen flex items-center justify-center font-black uppercase text-[10px] tracking-[0.4em] text-[#22C55E] animate-pulse">
        Master Data Syncing...
      </div>
    );
  if (!product)
    return (
      <div className="text-center p-10 font-black uppercase text-red-500">
        Product Not Found!
      </div>
    );
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-20 font-sans">
      <button
        onClick={() => router.back()}
        className="flex items-center gap-2 text-gray-400 hover:text-[#062010] font-black uppercase text-[10px] tracking-widest cursor-pointer transition-all"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                <div className="grid grid-cols-4 gap-3">
                  {product.media.images.map((img, i) => (
                    <img
                      key={i}
                      src={img}
                      className="w-full aspect-square rounded-2xl object-cover border border-gray-100 hover:border-[#22C55E] transition-all cursor-zoom-in"
                      alt=""
                    />
                  ))}
                </div>
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="space-y-4">
            ...
            <h1 className="text-4xl font-black text-[#062010] uppercase tracking-tighter leading-none">
              {product.name}
            </h1>
            <div className="flex items-center gap-2 text-gray-400 text-[10px] font-bold uppercase tracking-tighter">
              <Hash size={12} /> System ID: {product._id} | <Globe size={12} />{" "}
              Slug: {product.slug}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
              ...
              <p className="text-xl font-black text-[#062010]">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
            <h4 className="text-[11px] font-black uppercase tracking-widest mb-6 flex items-center gap-2">
              <Activity size={18} className="text-[#22C55E]" /> Status &
              Attributes
            </h4>
            <div className="flex flex-wrap gap-3">
              <div
                className={`px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest border ${product.status?.isActive ? "bg-green-50 text-green-600 border-green-100" : "bg-red-50 text-red-600 border-red-100"}`}
              >
                {product.status?.isActive ? "Active" : "Inactive"}
              </div>
              <div
                className={`px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest border ${product.status?.isNew ? "bg-blue-50 text-blue-600 border-blue-100" : "bg-gray-50 text-gray-400 border-gray-100"}`}
              >
                New Arrival: {product.status?.isNew ? "YES" : "NO"}
              </div>
              <div
                className={`px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest border ${product.status?.isFeatured ? "bg-purple-50 text-purple-600 border-purple-100" : "bg-gray-50 text-gray-400 border-gray-100"}`}
              >
                Featured: {product.status?.isFeatured ? "YES" : "NO"}
              </div>
              <div
                className={`px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest border ${product.status?.isTodaySpecial ? "bg-orange-50 text-orange-600 border-orange-100 animate-pulse" : "bg-gray-50 text-gray-400 border-gray-100"}`}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                {[
                  { label: "Origin", val: product.shipping?.origin },
                  {
                    ...
                    val: `৳${product.shipping?.deliveryCharge}`,
                  },
                  {
                    label: "Estimate",
                    val: product.shipping?.estimatedDelivery,
                  },
                  { label: "Shelf Life", val: product.shipping?.shelfLife },
                  {
                    label: "Free Delivery",
                    val: product.shipping?.freeDelivery ? "Active" : "Inactive",
                  },
                  {
                    label: "Min Order",
                    val: `${product.inventory?.minOrderQuantity} ${product.inventory?.unit}`,
                  },
                ].map((item, idx) => (
                  <div
                    key={idx}
                    className="flex justify-between items-center text-xs font-bold border-b border-gray-50 pb-2 last:border-0"
                  >
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
              <h4 className="text-[11px] font-black uppercase tracking-widest mb-6 flex items-center gap-2">
                <ShieldCheck size={18} className="text-[#22C55E]" /> Features &
                Tags
              </h4>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {product.content?.features?.map((f, i) => (
                      <span
                        key={i}
                        className="px-3 py-1.5 bg-gray-50 text-[#062010] text-[10px] font-bold rounded-lg border border-gray-100"
                      >
                        {f}
                      </span>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {product.content?.tags?.map((t, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black rounded-full border border-emerald-100"
                      >
                        #{t}
                      </span>
                    ))}
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {product.variants.map((v, i) => (
                  <div
                    key={i}
                    className="p-4 bg-gray-50 rounded-2xl border border-gray-100 flex justify-between items-center"
                  >
                    <div>
          ...
          <div className="bg-white p-10 rounded-[3.5rem] border border-gray-100 shadow-sm relative overflow-hidden">
            ...
            <h4 className="text-[11px] font-black uppercase tracking-widest mb-6 flex items-center gap-2">
              <Info size={18} className="text-[#22C55E]" /> Technical
              Description
            </h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-[#062010] p-8 rounded-[2.5rem] text-white flex items-center gap-5 shadow-2xl shadow-green-900/20">
              ...
              <div>
                <p className="text-[9px] font-black uppercase text-gray-400 tracking-widest opacity-60">
                  System Operator
                </p>
                <p className="text-lg font-black text-white leading-none mt-1">
                  {product.metadata?.addedBy?.name}
                </p>
