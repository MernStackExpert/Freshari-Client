        ...
        {loading
          ? Array(3)
              ...
              .map((_, i) => (
                <div
                  key={i}
                  className="h-72 bg-gray-100 rounded-[3rem] animate-pulse"
                ></div>
              ))
          : filteredBanners.map((banner) => (
              <motion.div
                layout
                key={banner._id}
                className="bg-white p-4 rounded-[2.5rem] border border-gray-100 shadow-sm relative group overflow-hidden flex flex-col h-full"
              >
                <div className="relative h-48 rounded-[2rem] overflow-hidden border border-gray-50">
                  ...
                  <div className="absolute top-3 left-3 flex gap-2">
                    {banner.isRight && (
                      <span className="bg-blue-500 text-white text-[7px] font-black uppercase px-2 py-1 rounded-md shadow-lg">
                        Right Side
                      </span>
                    )}
                    {banner.isDeals && (
                      <span className="bg-[#22C55E] text-[#062010] text-[7px] font-black uppercase px-2 py-1 rounded-md shadow-lg">
                        Flash Deal
                      </span>
                    )}
                  </div>
                ...
                <div className="p-4 flex-grow">
                  <h3 className="text-sm font-black text-[#062010] uppercase tracking-tight truncate">
                    {banner.title || "Untitled Component"}
                  </h3>
                ...
                <div className="p-2 flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 bg-gray-50 text-gray-400 py-3 rounded-xl hover:bg-[#062010] hover:text-[#22C55E] transition-all cursor-pointer flex justify-center border border-transparent hover:border-[#22C55E]/30"
                  >
                    <Edit3 size={16} />
                  </button>
                  <button
                    onClick={() => handleDelete(banner._id)}
                    className="flex-1 bg-red-50 text-red-400 py-3 rounded-xl hover:bg-red-500 hover:text-white transition-all cursor-pointer flex justify-center"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            <motion.div
              ...
              className="absolute inset-0 bg-[#062010]/80 backdrop-blur-md cursor-pointer"
            />
            <motion.div
              initial={{ y: 50, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              exit={{ y: 50, opacity: 0 }}
              className="bg-white w-full max-w-xl rounded-[3.5rem] overflow-hidden shadow-2xl relative z-10 font-sans"
            >
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-[9px] font-black uppercase text-gray-400 tracking-widest mt-1">
                      Configure Banner Logic
                    </p>
                  </div>
                  <button
                    onClick={() => setIsModalOpen(false)}
                    className="p-2.5 bg-gray-50 rounded-full hover:bg-red-50 hover:text-red-500 transition-all cursor-pointer"
                  >
                    <X size={20} />
                  </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="space-y-4 bg-gray-50/50 p-6 rounded-[2.5rem] border border-gray-100">
                      ...
                      <div className="flex bg-white rounded-lg p-1 shadow-sm">
                        {["upload", "url"].map((t) => (
                          <button
                            ...
                            type="button"
                            onClick={() => setImgTab(t)}
                            className={`px-4 py-1 text-[8px] font-black uppercase rounded ${imgTab === t ? "bg-[#062010] text-[#22C55E]" : "text-gray-400"}`}
                          >
                            {t}
                          </button>
                        ))}
                    ...
                    {imgTab === "upload" ? (
                      ...
                      <input
                        ...
                        className="w-full bg-white rounded-2xl p-4 text-xs font-bold outline-none border border-gray-100"
                      />
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full bg-gray-50 rounded-2xl p-4 text-sm font-bold outline-none border border-transparent focus:border-[#22C55E]/20"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-gray-400 ml-1">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button
                      type="button"
                      onClick={() =>
                        setFormData({ ...formData, isRight: !formData.isRight })
                      }
                      className={`p-4 rounded-2xl border flex items-center justify-between transition-all ${formData.isRight ? "bg-blue-50 border-blue-200 text-blue-600" : "bg-gray-50 border-transparent text-gray-400"}`}
                    >
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setFormData({ ...formData, isDeals: !formData.isDeals })
                      }
                      className={`p-4 rounded-2xl border flex items-center justify-between transition-all ${formData.isDeals ? "bg-emerald-50 border-emerald-200 text-[#22C55E]" : "bg-gray-50 border-transparent text-gray-400"}`}
                    >
                      <span className="text-[9px] font-black uppercase tracking-widest">
                        Flash Deals
                      </span>
                  ...
                  <button
                    disabled={uploading}
                    type="submit"
                    className="w-full bg-[#22C55E] text-[#062010] py-5 rounded-[2rem] font-black uppercase text-xs tracking-[0.2em] hover:shadow-2xl hover:shadow-green-200 transition-all cursor-pointer disabled:opacity-50"
                  >
                    {uploading
                      ? "Uploading Asset..."
                      : editId
