...
import { 
  DollarSign, ShoppingBag, Package, Activity, Layers, TrendingUp 
} from "lucide-react";
...
const DashboardPage = () => {
  ...
  const cardData = [
    { title: "Total Revenue", value: `৳${stats?.totalRevenue || 0}`, icon: <DollarSign />, color: "text-emerald-500", bg: "bg-emerald-500/10" },
    { title: "Total Orders", value: stats?.totalOrders || 0, icon: <ShoppingBag />, color: "text-blue-500", bg: "bg-blue-500/10" },
    { title: "Total Products", value: stats?.totalProducts || 0, icon: <Package />, color: "text-orange-500", bg: "bg-orange-500/10" },
    { title: "Pending Orders", value: stats?.pendingOrders || 0, icon: <Activity />, color: "text-red-500", bg: "bg-red-500/10" },
  ];
  ...
  return (
    <div className="space-y-10 pb-10">
      <div>
        <h1 className="text-3xl font-black text-[#062010] tracking-tighter uppercase">Freshari Insights</h1>
        <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.3em] mt-1">Live Database Statistics</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {cardData.map((item, idx) => (
          ...
          >
            ...
            <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest">{item.title}</p>
            <h2 className="text-3xl font-black text-[#062010] mt-1">{item.value}</h2>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm">
          <h3 className="text-xs font-black uppercase tracking-widest mb-8">Inventory Bar Analysis</h3>
        ...
        <div className="bg-[#062010] p-8 rounded-[3rem] text-white shadow-2xl">
          <h3 className="text-xs font-black uppercase tracking-widest mb-8">Operational Radar</h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="80%" data={distributionData}>
                <PolarGrid stroke="#ffffff10" />
                <PolarAngleAxis dataKey="name" tick={{ fill: '#94a3b8', fontSize: 10 }} />
                <Radar name="Count" dataKey="count" stroke="#22C55E" fill="#22C55E" fillOpacity={0.6} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm flex flex-col items-center">
          <h3 className="text-xs font-black uppercase tracking-widest mb-4 w-full">Order Ratio</h3>
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={orderStatusData} innerRadius={60} outerRadius={85} paddingAngle={8} dataKey="value">
          ...
          <div className="flex gap-6 mt-4">
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-blue-500"/> <span className="text-[10px] font-bold uppercase text-gray-400">Total</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-orange-500"/> <span className="text-[10px] font-bold uppercase text-gray-400">Pending</span></div>
          </div>
        </div>

        <div className="lg:col-span-2 bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm overflow-x-auto">
          <h3 className="text-xs font-black uppercase tracking-widest mb-8">Database Resource Count</h3>
  ...
  <tr className="border-b border-gray-50">
    ...
    <td className="py-4 text-right">{stats?.totalCategories || 0}</td>
  </tr>
  <tr className="border-b border-gray-50">
